...
validate_syntax() {
    ...
    find "${WORK_DIR}/pages" -name '*.adoc' -exec \
        perl -pi -e 's|include::partial\$|include::../partials/|g' {} +
...
validate_xrefs() {
    ...
    while IFS= read -r -d '' file; do
        ...
        local matches
        matches=$(grep -noE 'xref:[^[]+\[' "$file" 2>/dev/null) || continue
        ...
        while IFS=: read -r lineno match; do
            ...
            # Skip cross-component xrefs, attribute-substituted targets, and same-page anchors
            if [[ "$target" == *:* || "$target" == "{"* || -z "$target" ]]; then
                continue
            fi
...
validate_includes() {
    ...
    while IFS= read -r -d '' file; do
        local relpath="${file#"${REPO_ROOT}/"}"
        local matches
        matches=$(grep -noE 'include::[^[]+\[' "$file" 2>/dev/null) || continue
...
validate_nav() {
    ...
    for navfile in "${NAV_FILE}" "${PARTIALS_DIR}/contents.adoc"; do
        ...
        local matches
        matches=$(grep -noE 'xref:[^[]+\[' "$navfile" 2>/dev/null) || continue
